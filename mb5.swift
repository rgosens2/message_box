...
func createNextBox() {
    ...
    let window = BoxWindow(title: "Box \(boxCount + 1)", x: x, y: y, boxNumber: boxCount + 1)
    window.makeKeyAndOrderFront(nil)
    window.level = .normal  // ensure it's at normal level
    window.orderFrontRegardless()  // force front
